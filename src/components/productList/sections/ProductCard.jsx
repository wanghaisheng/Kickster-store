    ...
    return (
        ...
        <NavLink to={`/product/${product.id}`} className=' flex-shrink-0 w-[31.6%] h-[480px] overflow-hidden'>
            ...
            <div className="product-details w-full mt-1 leading-tight">
                ...
                <p className="product-price mt-3 text-[1.1rem] txt-medium flex gap-[1ch] items-center">{`₹ ${priceCorrection(Math.round(priceStringToInt(product.price) - (priceStringToInt(product.price) * product.discount) / 100))}.00`}<span className='text-orange-800 text-[0.95rem]'>{`-${product.discount}%`}</span></p>
            </div>
        </NavLink>
    )
}
